...
document.querySelector('.burger').addEventListener('click', function () {
	...
	document.querySelector('.header__contact').classList.toggle('header__contact--open');
});

// Close all submenus when clicking outside of them
document.addEventListener('click', function (event) {
	let target = event.target;
	if (!target.classList.contains('submenu') && !target.classList.contains('menu__link')) {
		let allSubMenus = document.querySelectorAll('.submenu');
...
document.addEventListener('click', function (event) {
	...
	if (!burger.contains(event.target) && !menu.contains(event.target) && burger.classList.contains('active')) {
		burger.classList.remove('active');
		menu.classList.remove('open');
		document.querySelector('.header__contact').classList.remove('header__contact--open');
	}
});